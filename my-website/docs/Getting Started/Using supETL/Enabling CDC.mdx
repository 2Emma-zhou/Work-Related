...
1. Open the cinfiguration file of the MySQL.
...
Before start, check whether the archive mode is enabled.
```
...
2. Check whether the redo log is created.
```
show parameter DB_RECOVERY_FILE_DEST
```
3. Check whether the tablespace of the current user is permanent tablespace.
:::info
...
```
4. Create permanent tablespace file.
:::info
...
5. Check whether the supplymental log is enabled.
```
SELECT supplemental_log_data_min, supplemental_log_data_pk,
supplemental_log_data_all FROM v$database;
```
6. (optional) When the result of the last step is **NO NO**, run the following statement to enable supplymental log.
```
ALTER DATABASE ADD SUPPLEMENTAL LOG DATA (ALL) COLUMNS;
```
7. Grant CDC permission.
- Oracle 11g
```
...
![](https://wordpressfreezonex.oss-cn-hangzhou.aliyuncs.com/supETL/45.png)

3. Check whether the database has enabled CDC.
```
select is_cdc_enabled, name from sys.databases
```
![](https://wordpressfreezonex.oss-cn-hangzhou.aliyuncs.com/supETL/46.png)
...
select is_cdc_enabled, name from sys.databases
```
![](https://wordpressfreezonex.oss-cn-hangzhou.aliyuncs.com/supETL/47.png)

6. Check whether the tables you want to synchronize data from have enabled CDC.
```
select name,is_tracked_by_cdc from sys.tables
```
![](https://wordpressfreezonex.oss-cn-hangzhou.aliyuncs.com/supETL/48.png)
...
![](https://wordpressfreezonex.oss-cn-hangzhou.aliyuncs.com/supETL/49.png)
