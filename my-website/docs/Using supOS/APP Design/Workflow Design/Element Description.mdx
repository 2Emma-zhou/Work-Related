...
- Place it on a task, and if the task is finished before the set time, the timer becomes invalid; if not, a branch task is triggered.
...
- <strong>Interrupt</strong>: The current task executor cannot proceed the workflow after reaching the set time, and only branches can.
