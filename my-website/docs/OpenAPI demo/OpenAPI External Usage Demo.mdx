...
## Operation Process
![](https://wordpressfreezonex.oss-cn-hangzhou.aliyuncs.com/openAPI/5.png)
...
4. Click **Donwload** on the pop-up window to download the authorization file.
 ![](https://wordpressfreezonex.oss-cn-hangzhou.aliyuncs.com/openAPI/6.png)
...
Use the following code to calculate the signature for using open API to get supOS platform data in third-party platforms.
1. Copy the following code in your IDE, change the AK/SK information to what you got from the supOS platform.
- Node.js version
```
...
```
2. Run the code and get the signature.<br/>
The result should be like the following examples.
- Node.js version
```
...
1. In your IDE, create another file, and then copy the following code.
- Node.js version

<ul><ul>
<li>GET/DELETE request</li></ul></ul>
...
function test() {
    ...
    methodName = 'GET' //or DELETE, depends on the openAPI
    let queryJson = {
        "current": "1",
        "pageSize": "500"
    };
...
const rp = require('request-promise');
function test() {
    let openApiUri = '/open-api/supos/oodm/v2/template/emma_sales/sales/instance/sales1'; //emma_sales=template namespace, sales=template alias, sales1=instance alias
    methodName = 'POST' //or PUT, depends on the openAPI
    let queryJson = {};
    let headerJson = {
        'Content-Type': 'application/json;charset=utf-8'
    };
 let wholeUrl=hosted.supos+openApiUri;
    ...
    var options = {
        ...
        uri: wholeUrl,
        headers: headerJson,
        body: { //body parameters of the openAPI
            "appAccessMode" : "PUBLIC",
            "attributeValues" : [ {
              "attributeName" : "test",
              "attributeNamespace" : "emma_sales",
              "attributeValue" : "123"
            } ],
            "code" : "xxxxx",
            "comment" : "edit entity instance",
...
```
2. Run the code to get corresponding data from supOS platform.